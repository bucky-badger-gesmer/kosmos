...
- Use the data-fetching model the project's router uses: `getStaticProps`/`getServerSideProps` on the Pages Router, server components and route handlers on the App Router
...
- Keep the default SWC compiler; tune the bundler configuration only when a measured build problem requires it
