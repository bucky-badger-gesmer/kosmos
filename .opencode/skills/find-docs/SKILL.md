...
name: find-docs
description: >-
  Retrieves up-to-date documentation, API references, and code examples for a
  specific library, framework, SDK, CLI tool, or cloud service via Context7. Use
  when the answer depends on current library specifics that training data may have
  stale or wrong: API syntax and signatures, configuration options, version-specific
  behavior or migrations, setup/installation steps, and CLI usage -- especially for
  fast-moving libraries. Prefer this over web search for library documentation. Do
  NOT use it for general programming concepts, language syntax, algorithmic help, or
  questions not tied to a particular library's current API.
---
...
Works without authentication (lower rate limit). For higher limits, set
`CONTEXT7_API_KEY` in the environment:
...
This workspace's `.env` already holds a populated `CONTEXT7_API_KEY` (see the Env
section of the root `AGENTS.md`). Note that `.env` is not auto-loaded into the shell,
so `ctx7` runs unauthenticated until the key is exported — e.g. `source .env`, or run
`export CONTEXT7_API_KEY="$(grep -E '^CONTEXT7_API_KEY=' .env | cut -d= -f2-)"` before
the commands above.
